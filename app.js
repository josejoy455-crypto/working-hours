...
// Daily salary is always monthly salary divided by a fixed 30-day standard, regardless of
// how many days the actual calendar month has. This matches standard fixed-salary payroll practice.
const SALARY_DIVISOR_DAYS = 30;
...
function computeMonthStats(year, monthIdx) {
  ...
  // Count workable days (calendar days minus configured weekly-off days like Sunday)
  let workableDaysTotal = 0, workableDaysRemaining = 0, workableDaysPast = 0;
  for (let d = 1; d <= totalDaysInMonth; d++) {
    const key = `${year}-${String(monthIdx+1).padStart(2,"0")}-${String(d).padStart(2,"0")}`;
    if (!isWeeklyOff(key)) {
      workableDaysTotal++;
      if (isCurrentMonth && d < todayNum) workableDaysPast++;
      if (!isCurrentMonth || d >= todayNum) workableDaysRemaining++;
    }
  }
  ...
  keys.forEach(k => {
    ...
    if (entry.status === "leave") {
      leaveCount++;
      if (inToDate) leaveCountToDate++;
      return;
    }
    if (entry.status === "paid-leave") {
      paidLeaveCount++;
      if (inToDate) paidLeaveCountToDate++;
      return;
    }
    const { normalMin, otMin } = computeEntryMinutes(entry, k);
    if (normalMin > 0 || otMin > 0) workedDays++;
    normalMinTotal += normalMin;
    otMinTotal += otMin;
    if (inToDate) {
      if (normalMin > 0 || otMin > 0) workedDaysToDate++;
      normalMinToDate += normalMin;
      otMinToDate += otMin;
    }
  });

  const hSalary = hourlySalary();
  const shiftHours = shiftDurationMinutes() / 60;

  const normalSalaryEarned = shiftHours > 0 ? (normalMinToDate / 60) * hSalary : 0;
  ...
  const currentSalaryEarned = normalSalaryEarned + otPay + paidLeaveSalary;

  // Expected salary: fixed daily salary rate applied across workable days elapsed so far
  const expectedSalary = workableDaysPast * dSalary + (isCurrentMonth ? (todayNum <= totalDaysInMonth && !isWeeklyOff(todayKey()) ? dSalary : 0) : 0);

  // Maximum possible salary from here forward: fixed monthly salary (already "owed" regardless of
  // days worked) + OT already earned + the best-case OT still achievable on remaining workable days.
  const maxOTMin = maxOTMinutesPerDay();
  const maxOTPayRemaining = workableDaysRemaining * (maxOTMin / 60) * config.otRate;
  ...
  // Remaining to earn = max possible ceiling minus what's been earned so far (base + OT + paid leave)
  ...
  return {
    ...
    workableDaysTotal, workableDaysRemaining,
...
function renderDashboard() {
  ...
  const todayEarnings = (todayCalc.normalMin / 60) * stats.hSalary;
...
function renderEntryList() {
  ...
  const stats = { hSalary: hourlySalary() };
  list.innerHTML = keys.map(k => {
    ...
    let timesStr = "—", badges = "";
    const amount = dayPay(k, stats.hSalary);
    if (e.status === "leave") {
    ...
    return `
      <div class="entry-item" data-key="${k}">
        <div class="entry-date"><span class="dnum">${dnum}</span><span class="dmon">${dmon}</span></div>
...
function openEntryModal(dateKey) {
  ...
  const e = entries[dateKey] || { status: "worked", clockIn: config.workStart, clockOut: config.workEnd, notes: "" };
  $("#entryDate").value = dateKey;
  entryStatus.value = e.status || "worked";
  $("#entryClockIn").value = e.clockIn || (isNew ? config.workStart : "");
  $("#entryClockOut").value = e.clockOut || (isNew ? config.workEnd : "");
...
function showCalDetail(key) {
  ...
  if (e.status === "leave") {
    detail.innerHTML = `<strong>${label}</strong><br>Unpaid leave · No salary earned for this day (${fmtMoney(dailySalary())} not added, no extra penalty)${e.notes ? "<br>Note: " + escapeHtml(e.notes) : ""}`;
  } else if (e.status === "paid-leave") {
    detail.innerHTML = `<strong>${label}</strong><br>Paid leave · Full day salary ${fmtMoney(dailySalary())}${e.notes ? "<br>Note: " + escapeHtml(e.notes) : ""}`;
  } else if (e.status === "ot-only") {
    const { otMin } = computeEntryMinutes(e, key);
    const pay = (otMin/60) * config.otRate;
    detail.innerHTML = `<strong>${label}</strong> · overtime only<br>No regular shift · Worked ${minutesToHM(otMin)} OT<br>Earned: <strong>${fmtMoney(pay)}</strong>${e.notes ? "<br>Note: " + escapeHtml(e.notes) : ""}`;
  } else {
    const { normalMin, otMin } = computeEntryMinutes(e, key);
    const hS = hourlySalary();
    const pay = (normalMin/60)*hS + (otMin/60)*config.otRate;
    const offNote = off ? " · off-day, full OT" : "";
...
function renderReports() {
  ...
  if (activeReportType === "daily") {
    ...
    body.innerHTML = keys.length ? keys.map(k => reportLineForDay(k, hS)).join("") : emptyMsg();
  } else if (activeReportType === "weekly") {
    const weeks = groupByWeek(keys);
    body.innerHTML = weeks.length ? weeks.map(w => {
      const totalPay = w.keys.reduce((sum, k) => sum + dayPay(k, hS), 0);
    ...
    body.innerHTML = months.length ? months.map(mo => {
      const totalPay = mo.keys.reduce((sum, k) => sum + dayPay(k, hS), 0);
